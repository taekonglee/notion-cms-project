...
- [x] Next.js `<Image>` 컴포넌트 적용 확인 (모든 이미지)
  - `width`, `height` 또는 `fill` 속성 설정
  - `priority` 속성: LCP 대상 이미지(홈 페이지 상단)에 적용
- [x] 불필요한 클라이언트 컴포넌트 Server Component 전환 검토
- [x] `next/font`로 폰트 최적화 (기존 Geist 폰트 설정 확인)
- [ ] 코드 분할: 동적 import(`next/dynamic`) 검토 (Notion 렌더러 등 무거운 컴포넌트)

#### 반응형 최종 점검 — [PRD F005]

- [x] 모바일(375px) 레이아웃 전체 페이지 확인
- [x] 태블릿(768px) 레이아웃 확인
- [x] 데스크탑(1280px) 레이아웃 확인
- [ ] 터치 인터랙션, 탭 이동, 가독성(폰트 크기) 점검

#### 다크모드 최종 점검 — [PRD F006]

- [x] 시스템 다크모드 자동 전환 확인
- [x] 수동 토글 시 블로그 컴포넌트 모두 올바른 테마 색상 적용 확인
...
- [x] Google PageSpeed Insights (Mobile) 실행
  - LCP: 2.5초 이하 목표 — [PRD 성공 지표] (로컬 측정: 2.6초, Vercel 배포 후 재측정 필요)
  - 모바일 점수: 90점 이상 목표 — [PRD 성공 지표] (달성: Mobile 97점, Desktop 100점)
...
| **F005** | 반응형 디자인 (375px~) | Phase 3, 5 | ☑ 완료 (375/768/1280px E2E 스크린샷 확인) |
| **F006** | 다크모드 지원 (next-themes 재사용) | Phase 3, 5 | ☑ 완료 (수동 토글 + 시스템 전환 E2E 확인) |
